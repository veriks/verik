import { CrosscheckError } from './errors.js';
 ...
 * "Error: CrosscheckError: Not a Git repository" — the class name is an
 * implementation detail and the doubled prefix reads like a bug.
 */
export function formatError(err: unknown): string {
  if (err instanceof CrosscheckError) return err.message;
...
export function exitCodeFor(err: unknown): number {
  return err instanceof CrosscheckError ? err.exitCode : 1;
}
