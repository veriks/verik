...
interface CheckResult { label: string; status: 'ok' | 'warn' | 'fail'; detail?: string; }

async function check(label: string, fn: () => Promise<'ok' | 'warn' | { fail: string } | { warn: string }>): Promise<CheckResult> {
  try {
    const result = await fn();
    if (result === 'ok')   return { label, status: 'ok' };
    if (result === 'warn') return { label, status: 'warn' };
    if (typeof result === 'object' && 'fail' in result) return { label, status: 'fail', detail: result.fail };
    if (typeof result === 'object' && 'warn' in result) return { label, status: 'warn', detail: result.warn };
...
export function buildDoctorCommand(): Command {
  return new Command('doctor')
    ...
    .action(async (options: { json?: boolean }) => {
      ...
      results.push(await check('.crosscheck/ directory exists', async () => {
        try { await access(join(repoRoot, '.crosscheck')); return 'ok'; }
        catch { return { warn: 'run `crosscheck init` to create it' }; }
      }));

      // 3. Config parses
      results.push(await check('config.json is valid', async () => {
        try {
      ...
      // 4. Policy parses
      results.push(await check('policy.json is valid', async () => {
        try {
      ...
      results.push(await check('ANTHROPIC_API_KEY is set', async () => {
        if (apiKey) return 'ok';
      ...
      if (apiKey) {
        results.push(await check('API key is accepted by Anthropic', async () => {
          try {
            ...
            if (e instanceof Anthropic.AuthenticationError) {
              return { fail: 'API key is invalid or expired — generate a new key at console.anthropic.com' };
            }
        ...
        let config;
        try { config = await loadConfig(repoRoot); } catch { /* already caught above */ }
        if (config) {
          const client = new Anthropic({ apiKey, maxRetries: 0 });
          for (const [stage, model] of [
            ...
            results.push(await check(`Model for ${stage} (${model}) is accessible`, async () => {
              try {
                await client.messages.create({
                  model,
                  max_tokens: 1,
                ...
                if (e instanceof Anthropic.NotFoundError) {
                  return { fail: `Model "${model}" not found — check CROSSCHECK_MODEL_${stage.toUpperCase()} or config.json` };
                }
                if (e instanceof Anthropic.AuthenticationError) {
      ...
      results.push(await check('Builder extra commands pass allowlist', async () => {
        try {
      ...
      if (failed.length === 0 && warned.length === 0) {
        ...
        console.log(c() ? chalk.red(`${failed.length} check(s) failed.`) : `${failed.length} check(s) failed.`);
        process.exit(1);
      } else {
        console.log(c() ? chalk.yellow(`${warned.length} warning(s).`) : `${warned.length} warning(s).`);
      }
    });
}
