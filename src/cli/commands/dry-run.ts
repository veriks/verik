...
export function buildDryRunCommand(): Command {
  return new Command('dry-run')
    ...
    .action(async (commandArgs: string[], options: { intent?: string }) => {
      try {
        ...
        console.log(`  Dirty: ${info.isDirty ? yellow('yes (pre-existing changes exist)') : green('no')}`);
        console.log();
        ...
        const diff = await computeDiff(root, snapshot, config.verification.maxDiffBytes, config.verification.includeUntrackedFiles);

        if (diff.changedFiles.length > 0) {
          console.log(yellow('⚠ Repository has uncommitted changes that would be treated as pre-existing:'));
          for (const f of diff.changedFiles.slice(0, 8)) {
            console.log(dim(`  ${f.changeType}: ${f.path}`));
          }
          if (diff.changedFiles.length > 8) console.log(dim(`  ... and ${diff.changedFiles.length - 8} more`));
          console.log();
        }
        ...
        console.log(`  Block at: ${policy.blockAtSeverity} severity ≥ ${policy.minimumBlockingConfidence * 100}% confidence`);
        console.log();

        // Builder
        console.log(bold('Builder (deterministic)'));
        const detection = detectProject(root);
        console.log(`  Project: ${detection.projectTypes.join(', ')}${detection.packageManager ? ` · ${detection.packageManager}` : ''}`);
        ...
        const sampleDiff = { ...diff, changedFiles: [], commandIntroducedPaths: [], preExistingChangedPaths: [] };
        const ctx = await selectContext({
          repoRoot: root, diff: sampleDiff,
        ...
        if (overrides.length > 0) {
          ...
          for (const o of overrides) {
            const scope = [o.ruleId && `rule:${o.ruleId}`, o.filePath && `path:${o.filePath}`, o.titlePattern && `title:/${o.titlePattern}/`].filter(Boolean).join(', ');
